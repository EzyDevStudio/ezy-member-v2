// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'member_profile_hive.dart';

// **************************************************************************
// TypeAdapterGenerator
// **************************************************************************

class MemberProfileHiveAdapter extends TypeAdapter<MemberProfileHive> {
  @override
  final int typeId = 0;

  @override
  MemberProfileHive read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read()};
    return MemberProfileHive(id: fields[0] as int, memberCode: fields[1] as String, name: fields[2] as String);
  }

  @override
  void write(BinaryWriter writer, MemberProfileHive obj) {
    writer
      ..writeByte(3)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.memberCode)
      ..writeByte(2)
      ..write(obj.name);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) || other is MemberProfileHiveAdapter && runtimeType == other.runtimeType && typeId == other.typeId;
}
